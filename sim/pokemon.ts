	...
	*/
	...
	ignoringItem() {
		return !!(
			...
			(!this.getItem().ignoreKlutz && this.hasAbility('powerpromise')) ||
	...
	getDynamaxRequest(skipChecks?: boolean) {
		...
		if (!skipChecks) {
			...
			if (
				...
				this.getItem().zMove || this.canMegaEvo || this.canTerastallize || this.terastallized
			) {
				return;
			}
	...
	transformInto(pokemon: Pokemon, effect?: Effect) {
		...
		if (pokemon.fainted || this.illusion || pokemon.illusion || (pokemon.volatiles['substitute'] && this.battle.gen >= 5) ||
			...
			species.name === (['Ogerpon', 'Terapagos'].includes(species.baseSpecies) &&
