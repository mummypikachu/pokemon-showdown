...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
	...
	aquastep: {
		...
		pp: 10,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, dance: 1, kick: 1 },
		secondary: {
			chance: 100,
			self: {
				boosts: {
	...
	nihillight: {
		...
		onTry(source) {
			if (source.species.name === 'Zygarde-Mega') return;
			this.hint("Only a Pokemon whose form is Zygarde Mega can use this move.");
			if (['Zygarde-Complete', 'Zygarde', 'Zygarde-10%'].includes(source.species.name)) {
				this.attrLastMove('[still]');
	...
	glacierstream: {
		...
		priority: 0,
		flags: { reflectable: 1 },
		sideCondition: 'glacierstream', // Change the side condition name to 'icyrock'
		condition: {
			// This is a side condition for the Icetype hazard
			onSideStart(side) {
				this.add('-sidestart', side, 'move: Glacier Stream');
			},
			onEntryHazard(pokemon) {
				if (pokemon.hasItem('heavydutyboots')) return;
				const typeMod = this.clampIntRange(pokemon.runEffectiveness(this.dex.getActiveMove('glacierstream')), -6, 6);
	...
	mortalspin: {
		...
		onAfterHit(target, pokemon) {
			...
			const sideConditions = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge', 'glacierstream'];
	...
	roleplay: {
		...
		onTryHit(target, source) {
			...
			if (target.getAbility().flags['cantsuppress'] || additionalBannedTargetAbilities.includes(target.ability) ||
	...
	skyuppercut: {
		...
		pp: 15,
		priority: 0,
		flags: { contact: 1, protect: 1, mirror: 1, punch: 1, jab: 1 },
		target: "normal",
		type: "Fighting",
		contestType: "Cool",
	},
	...
	trasheater: {
		...
		onHit(pokemon) {
			...
			if (success) this.add('-activate', pokemon, 'move: Trash Eater');
		},
		target: "self",
		type: "Poison",
	},
	...
	contraryfield: { // do not ever give this move to anything. this solely exists to make Mega Malamar work!
		...
		condition: {
			...
			onStart(field, source) {
				this.add('-fieldstart', 'move: contraryfield');
				for (const side of this.sides) {
					for (const pokemon of side.active) {
						if (pokemon?.hp) pokemon.addVolatile('contraryfield');
					}
				}
			},
			onSwitchIn(pokemon) {
				pokemon.addVolatile('contraryfield');
			},
			onEnd() {
				for (const side of this.sides) {
					for (const pokemon of side.active) {
						if (pokemon?.volatiles['contraryfield']) {
							pokemon.removeVolatile('contraryfield');
						}
					}
				}
			},
