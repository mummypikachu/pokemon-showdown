...
export const Abilities: {[abilityid: string]: AbilityData} = {
	...
	drought: {
		onStart(source) {
			...
			this.field.setWeather('sunny day');
		},
		name: "Drought",
		rating: 4,
		num: 70,
	...
	forecast: {
        ...
        onWeatherChange(pokemon) {
          ...
          // Override with held weather rock if applicable
				switch (pokemon.item) {
					case 'heatrock':
	...
	goodaspyrite: {
		...
		onModifyMove(move) {
			if (move.category !== "Status") {
				this.debug('Adding Pyrite flinch');
		...
		onDamagingHit(damage, target, source, move) {
			...
			if (this.checkMoveMakesContact(move, source, target, !source.isAlly(target))) {
				...
				if (target.isAlly(source)) {
					...
					this.add('-activate', target, 'ability: Lingering Spirit', this.dex.abilities.get(sourceAbility).name, 'Lingering Spirit', '[of] ' + source);
				}
  ...
  onDamage(damage, target, source, effect) {
	...
	trace: {
		...
		onUpdate(pokemon) {
			...
			const additionalBannedAbilities = [
				...
				'noability', 'flowergift', 'forecast', 'hungerswitch', 'illusion', 'imposter', 'neutralizinggas', 'powerofalchemy', 'receiver', 'trace', 'zenmode', 'adaptation'
			];
	...
	artrobbery: {
		...
		onStart(pokemon) {
			...
			this.add('-message', `${pokemon.name} stole ${target.name} its ability!`);
		},
	},
};