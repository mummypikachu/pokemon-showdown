	...
	saveserverreplay(target, room) {
		...
		out.on('open', () => {
			out.write(
				...
				`<pre class="urlbox" style="word-wrap: break-word;">http://sigmatic-showdown.win:10000/${link}</pre>\n` +
		...
		if (target !== 'silent') {
			this.connection.popup(this.tr`Your replay has been uploaded! It's available at: http://sigmatic-showdown.win:10000/${link}`);
