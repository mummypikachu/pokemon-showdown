	...
	saveserverreplay(target, room) {
		...
		out.on('open', () => {
			out.write(
				...
				`<pre class="urlbox" style="word-wrap: break-word;">http://replay.sigmatic-showdown.online/files/${link}<button id="copyLinkBtn" onclick="copyLink()">Copy Link</button></pre>\n` +
		...
		if (target !== 'silent') {
			this.connection.popup(this.tr`Your replay has been uploaded! It's available at: http://replay.sigmatic-showdown.online/files/${link}`);
