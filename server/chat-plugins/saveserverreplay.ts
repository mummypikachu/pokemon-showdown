	...
	saveserverreplay(target, room) {
		...
		out.on('open', () => {
			out.write(
				...
				`<pre class="urlbox" style="word-wrap: break-word;">https://replay.sigmatic-showdown.online/files/${link}<button id="copyLinkBtn" onclick="copyLink()">Copy Link</button></pre>\n` +
				...
				'</div>\n' +
				'<script>\n' +
				`let daily = Math.floor(Date.now()/1000/60/60/24);document.write('<script src="https://play.sigmatic-showdown.online/js/replay-embed.js?version'+daily+'"></'+'script>');\n` +
				'</script>\n'
			);
			out.end();
		});
		...
		if (target !== 'silent') {
			this.connection.popup(this.tr`Your replay has been uploaded! It's available at: https://replay.sigmatic-showdown.online/files/${link}`);
