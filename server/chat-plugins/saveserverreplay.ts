	...
	saveserverreplay(target, room) {
		...
		out.on('open', () => {
			out.write(
				...
				`<pre class="urlbox" style="word-wrap: break-word;">https://github.com/mummypikachu/replay-keeper/${link}</pre>\n` +
		...
		if (target !== 'silent') {
			this.connection.popup(this.tr`Your replay has been uploaded! It's available at: https://github.com/mummypikachu/replay-keeper/${link}`);
