	...
	saveserverreplay(target, room) {
		...
		out.on('open', () => {
			out.write(
				...
				`<pre class="urlbox" style="word-wrap: break-word;">http://sigmatic-showdown.win:9000/#/file-browser/${link}</pre>\n` +
		...
		if (target !== 'silent') {
			this.connection.popup(this.tr`Your replay has been uploaded! It's available at: http://sigmatic-showdown.win:9000/#/file-browser/${link}`);
